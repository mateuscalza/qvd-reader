...
const readFileInChunks = async (filePath) => {
  return new Promise((resolve, reject) => {
    ...
    readStream.on('end', () => {
      const qvd = Buffer.concat(chunks)
      resolve(qvd)
    })
...
const main = async (file) => {
  const qvd = await readFileInChunks(file)
...
const getIndexTable = (header, buff, start, length) => {
  ...
  for (let i = start; i < start + length; i += rb) {
    ...
    for (let k = 0; k < header.campos.length; k++) {
      let result = 0
      if (header.campos[k].bitWidth === 0) result = 0
      else {
        result = bitsToInt32(
          row.slice(
            header.campos[k].bitOffset,
            header.campos[k].bitOffset + header.campos[k].bitWidth
          )
        )
      }
...
const bitsToInt32 = (arr) => {
  ...
  for (let i = 1; i < arr.length; i++) res += arr[i] * Math.pow(2, i)
  return res
}
...
const getHeader = (buff) => {
  ...
  const brtObj = convert.xml2js(bufxml.toString('utf-8'), {
    compact: true,
    spaces: 4,
  })
